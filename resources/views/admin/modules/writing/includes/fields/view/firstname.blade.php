<div id="{{ $id }}_field" class="card esi-card-writing mb-2 field_container">
    <div class="card-header esi-card-header-writing">
        ...
        <span>First Name : Field ID ({{ $id }})</span>
    ...
    <div class="card-body">    
        ...
        <input type="hidden" id="{{ $id }}_fieldType" name="{{ $id }}_fieldType" value="firstnamefield">           
        ...
        <h5 class="card-title font-weight-bold">@if(isset($display_meta['required'])) @if ($display_meta['required'] == "true")<span class="text-danger">*</span>@endif @endif {{ $label }}</h5>
        <div class="card-text">
            <input type="text" id="{{ $id }}_label" class="form-control form-control-sm bg-white" value="first name" disabled="disabled" >
            <div class="small">
