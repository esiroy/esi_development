<div id="{{ $id }}_field_row" class="row @if($display_meta['conditional_logic'] == 'true') {{ "cfLogic" }} @endif" 
    ...
    <div class="mb-3 text-left col-md-6 {{ $id }}_field_content">
        ...
        <input type="text" id="{{ $id }}" name="{{ 'firstname_' . $id }}" placeholder="{{ $label ?? '' }}" class="form-control firstnamefield bg-white" value="{{ Auth::user()->firstname ?? '' }}" readonly="readonly" @if ($display_meta['required']) {{ "required" }} @endif>
