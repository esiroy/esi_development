    ...
    <div class="mb-3 text-left col-md-6 {{ $id }}_field_content">    
        ...
        <select id="{{ $id }}" name="{{  str_replace(' ', '_', strtolower($label) ) .'_'. $id }}"  class="form-control" @if ($display_meta['required']) {{ "required" }} @endif>
            ...
            @foreach($tutors as $tutor) 
                @if ($tutor->user_id == 16800 || $tutor->user_id == 10290 || $tutor->user_id == 16557 || $tutor->user_id == 10043 || $tutor->user_id == 17592)
                    <!-- Lei (16800), Jess (10290), shane (16557), Sheryll (10043), Rena (17592)
                        (not selectable)
